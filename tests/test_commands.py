...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "yuanclaw is ready" in result.stdout
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("yuanclaw.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("yuanclaw.config.paths.get_cron_dir", return_value=cron_dir), \
         patch("yuanclaw.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("yuanclaw.cli.commands._make_provider", return_value=object()), \
         patch("yuanclaw.cli.commands._print_agent_response") as mock_print_response, \
         patch("yuanclaw.bus.queue.MessageBus"), \
         patch("yuanclaw.cron.service.CronService"), \
         patch("yuanclaw.agent.loop.AgentLoop") as mock_agent_loop_cls:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "yuanclaw.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("yuanclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("yuanclaw.config.paths.get_cron_dir", lambda: config_file.parent / "cron")
    monkeypatch.setattr("yuanclaw.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("yuanclaw.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("yuanclaw.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("yuanclaw.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("yuanclaw.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_gateway_uses_workspace_from_config_by_default(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("yuanclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr(
        "yuanclaw.cli.commands.sync_workspace_templates",
...
def test_gateway_workspace_option_overrides_config(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("yuanclaw.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("yuanclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr(
        "yuanclaw.cli.commands.sync_workspace_templates",
        lambda path: seen.__setitem__("workspace", path),
...
def test_gateway_uses_config_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("yuanclaw.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("yuanclaw.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("yuanclaw.session.manager.SessionManager", lambda _workspace: object())
