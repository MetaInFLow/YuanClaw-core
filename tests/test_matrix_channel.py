...
import yuanclaw.channels.matrix as matrix_module
from yuanclaw.bus.events import OutboundMessage
from yuanclaw.bus.queue import MessageBus
from yuanclaw.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from yuanclaw.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("yuanclaw.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "yuanclaw.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("yuanclaw.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "yuanclaw.channels.matrix.asyncio.create_task", _fake_create_task
    )
