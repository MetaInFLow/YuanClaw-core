"""Workspace access scope helpers."""
...
class WorkspaceScope:
    ...
    def metadata(self) -> dict[str, str]:
        return {"project_path": str(self.project_path), "access_mode": self.access_mode}
...
class WorkspaceScopeResolver:
    ...
    def default(self) -> WorkspaceScope:
        return default_workspace_scope(self.default_workspace, self.default_restrict_to_workspace)

    def for_message(self, msg: Any, session_metadata: Any) -> WorkspaceScope:
    ...
    def for_turn(
        ...
        *,
        ...
        has_explicit_scope = (
            ...
            and WORKSPACE_SCOPE_METADATA_KEY in message_metadata
        )
        if channel != self.scoped_channel and not has_explicit_scope:
            return self.default()
...
def workspace_sandbox_status(
    *,
    ...
    return WorkspaceSandboxStatus(
        ...
        provider="none",
        provider_label=_provider_label("none"),
        summary="Workspace restriction uses YuanClaw application-level guards.",
    )
...
def build_workspace_scope(
    ...
    *,
    ...
    return WorkspaceScope(
        ...
        sandbox_status=workspace_sandbox_status(restrict_to_workspace=restrict, workspace=root),
...
def validate_workspace_scope_payload(
    ...
    *,
    ...
    raw_path = raw.get("project_path") or raw.get("projectPath") or raw.get("path")
    ...
    raw_mode = raw.get("access_mode") or raw.get("accessMode")
    if raw_mode is None:
...
def current_tool_workspace(
    ...
    *,
    ...
    return ToolWorkspace(project_path=project_path, restrict_to_workspace=restrict, scope=scope)


def _env_system_provider(environ: dict[str, str] | None = None) -> str | None:
    env = environ if environ is not None else os.environ
    explicit_provider = env.get("YUANCLAW_WORKSPACE_SANDBOX_PROVIDER")
    enforced = env.get("YUANCLAW_WORKSPACE_SANDBOX_ENFORCED")
