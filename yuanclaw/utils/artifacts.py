...
from yuanclaw.utils.helpers import detect_image_mime, ensure_dir
...
def decode_image_data_url(data_url: str) -> tuple[bytes, str]:
    """Decode a base64 image data URL and return bytes plus detected MIME."""
...
def generated_image_tool_result(artifacts: list[dict[str, Any]]) -> str:
    """Return compact structured metadata exposed to the LLM."""
    return json.dumps(
        {
            "artifacts": artifacts,
            "next_step": (
