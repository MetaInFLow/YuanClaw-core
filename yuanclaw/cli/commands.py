...
app = typer.Typer(
    name="yuanclaw",
    help=f"{__logo__} yuanclaw - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from yuanclaw.config.paths import get_cli_history_path
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} yuanclaw v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """yuanclaw - Personal AI Assistant."""
    pass
...
@app.command()
def onboard():
    """Initialize yuanclaw configuration and workspace."""
    from yuanclaw.config.loader import get_config_path, load_config, save_config
    from yuanclaw.config.schema import Config
    ...
    console.print(f"\n{__logo__} yuanclaw is ready!")
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.yuanclaw/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print("  2. Chat: [cyan]yuanclaw agent -m \"Hello!\"[/cyan]")
    console.print("\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/yuanclaw#-chat-apps[/dim]")
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from yuanclaw.config.loader import load_config, set_config_path
...
def gateway(
    ...
    console.print(f"{__logo__} Starting yuanclaw gateway on port {port}...")
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show yuanclaw runtime logs during chat"),
):
...
@app.command()
def status():
    """Show yuanclaw status."""
    from yuanclaw.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} yuanclaw Status\n")
