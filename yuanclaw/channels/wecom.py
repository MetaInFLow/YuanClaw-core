...
from yuanclaw.config.schema import Base
...
class WeComChannel(BaseChannel):
    """WeCom channel using the SDK long-connection transport."""
    ...
    def default_config(cls) -> dict[str, Any]:
        """Return the default config shape for onboarding."""
    ...
    async def start(self) -> None:
        ...
        if not WECOM_AVAILABLE:
            logger.error("WeCom SDK not installed. Install wecom_aibot_sdk to enable this channel.")
            return
    ...
    async def _process_message(self, frame: Any, msg_type: str) -> None:
        """Process incoming message and forward to the bus."""
        try:
            if hasattr(frame, "body"):
                body = frame.body or {}
            elif isinstance(frame, dict):
