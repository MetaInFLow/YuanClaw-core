"""Native AWS Bedrock Converse provider."""
...
class BedrockProvider(LLMProvider):
    """Provider backed by AWS Bedrock Runtime's Converse APIs."""

    def __init__(
        self,
        api_key: str | None = None,
        api_base: str | None = None,
        default_model: str = "bedrock/global.anthropic.claude-sonnet-4-5",
        *,
    ...
    def _convert_messages(
        ...
        for msg in messages:
            role = msg.get("role")
            if role == "system":
                system.extend(self._system_blocks(msg.get("content")))
                continue
            if role == "tool":
            ...
            if role == "assistant":
                ...
                continue
            if role == "user":
                converted.append({"role": "user", "content": self._content_blocks(msg.get("content"))})
    ...
    def _build_kwargs(
        ...
        if self.extra_body:
            additional = _deep_merge(additional, self.extra_body)
        if additional:
